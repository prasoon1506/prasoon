...
if check_password():
 ...
 def create_target_vs_projected_graph(data):
    ...
    fig.update_layout(
        title='October 2024: Target vs Projected Sales',barmode='group',plot_bgcolor='rgba(255,255,255,0.1)',paper_bgcolor='rgba(0,0,0,0)',font_color='burlywood',title_font_color='burlywood',xaxis_title_font_color='burlywood',yaxis_title_font_color='burlywood',legend_font_color='burlywood',height=500)
 ...
 def prepare_data_for_pdf(data):
    ...
    for brand_data, brand_name in [(lc_data, 'LC'), (phd_data, 'PHD'), (lc_phd_data, 'LC+PHD')]:
        ...
        totals.append({'Zone': 'All India Total','Brand': brand_name,'Month Tgt (Oct)': total_month_tgt_oct,'Predicted Oct 2024': total_predicted_oct_2024,'Total Oct 2023': total_oct_2023,'YoY Growth': total_yoy_growth})
 ...
 def create_pdf(data):
    ...
    for _, row in pdf_data.iterrows():
        table_data.append([row['Zone'],row['Brand'],f"{row['Month Tgt (Oct)']:,}",f"{row['Predicted Oct 2024']:,}",f"{row['Total Oct 2023']:,}",f"{row['YoY Growth']:.2f}%"])
 ...
 def style_dataframe(df):
    ...
    for col in df.columns:
        if df[col].dtype in ['float64', 'int64']:
            ...
            styler.apply(lambda x: ['background-color: #f0f0f0'] * len(x), subset=[col])
    numeric_format = {'October 2024 Target': '{:.0f}','October Projection': '{:.2f}','October 2023 Sales': '{:.0f}','YoY Growth(Projected)': '{:.2f}%'}
 ...
 def main():
    ...
    file_path = "https://github.com/prasoon1506/prasoon/blob/main/College_project.xlsx"
    data, last_modified = load_data(file_path)
    st.write(f"Data last updated: {time.ctime(last_modified)}")
    if file_path is not None:
        features = ['Month Tgt (Oct)', 'Monthly Achievement(Sep)', 'Total Sep 2023', 'Total Oct 2023','Monthly Achievement(Apr)', 'Monthly Achievement(May)', 'Monthly Achievement(June)','Monthly Achievement(July)', 'Monthly Achievement(Aug)']
        X = data[features]
        ...
        with col1:
            ...
            fig_predictions1.update_layout(title='FY 2025 Till Sep', barmode='group', plot_bgcolor='rgba(255,255,255,0.1)', paper_bgcolor='rgba(0,0,0,0)', font_color='burlywood',xaxis_title_font_color='burlywood',yaxis_title_font_color='burlywood',title_font_color='burlywood',legend_font_color='burlywood')
        ...
        if not selected_data.empty:
            ...
            for i, month in enumerate(months):
                if month != 'Oct':
                    ...
                    fig_monthly_performance.data[i].y = [selected_data['Month Tgt (Oct)'].iloc[0],selected_data['Predicted Oct 2024'].iloc[0]]
